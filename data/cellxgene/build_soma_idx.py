...
def retrieve_soma_idx(query_name) -> List[str]:
    ...
    """
    # VALUE_FILTER = f"suspension_type != 'na' and self_reported_ethnicity == '{args.query_name}'"
    with cellxgene_census.open_soma(census_version=VERSION) as census:
        obs_reader = census["census_data"]["homo_sapiens"].obs.read(
            value_filter=VALUE_FILTER[query_name],
            column_names=["soma_joinid"]
        )
        # 先转成 list，强制加载所有 chunk
        tables = list(obs_reader)
        if len(tables) == 0:
            cell_metadata = pa.Table.from_pydict({"soma_joinid": pa.array([])})
        else:
            cell_metadata = pa.concat_tables(tables)
